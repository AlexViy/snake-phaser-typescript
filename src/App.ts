...
class SimpleGame {
	...
	create() {
		...
		const gameCanvas = < HTMLCanvasElement > document.getElementById("content");
		...
		function drawSnakePart(snakePart: ISnakePart) {
		...
		function isFoodOnSnake(part: ISnakePart) {
		...
		function main() {
			...
			gameIsRunning = window.setTimeout(function onTick() {
				clearCanvas();
				advanceSnake();
				drawFood();
				drawSnake();
